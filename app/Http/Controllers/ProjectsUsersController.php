...
class ProjectsUsersController extends BaseController
{
    function getAll(\Illuminate\Http\Request $request)
    {
        ...
        $project_users = ProjectsUsers::where("project_id", $request->project_id)->leftJoin("users", "users.id", "=", "projects_users.user_id")->distinct()->selectRaw("projects_users.status,
                ...
                projects_users.shared_user_id")->orderByRaw("FIELD(status , 'owner', 'pending', 'accepted') ASC")->get();
    ...
    function getPendning($email, \Illuminate\Http\Request $request)
    {
